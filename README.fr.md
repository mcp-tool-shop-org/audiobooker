...
| `voices` · `chapters` · `speakers` · `speakers merge` · `info` · `status` · `cache` · `emotions` · `pronunciation` · `completion` | Vérifiez et gérez (`speakers merge <from> <to>` regroupe les noms en double dans un seul emplacement). |
