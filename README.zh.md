...
Audiobooker 能够检测对话，为每个角色分配独特的语音，推断情感，让您在渲染单个音轨之前，可以查看并更正所有内容，然后根据规范对结果进行优化——因此，输出的是一本*可提交*的有声读物，而不仅仅是生成的音频。
...
```
只需挂载一个即可，并且 `--rm` 是安全的：渲染缓存位于项目文件旁边，而不是位于主目录中，因此重新运行会从上次中断的地方继续，而不是重新合成整个内容。

<details>
<summary>Container details — tags, the cache, and file ownership</summary>

- 标记了 `latest`、`2`、`2.1` 以及确切的版本，并在每次发布时推送到 GHCR。
- 入口点是 `audiobooker`，因此请在镜像名称之后直接传递子命令——不要重复程序名称。
- 缓存存储在 `<book-dir>/.audiobooker/cache`。这就是为什么一个绑定挂载就足以实现持久化；如果丢失了它，意味着需要再次支付整个 TTS 运行的费用，而不仅仅是重新复用。
- `/ext` 是一个可选的第二个挂载点，仅用于提供您自己的 TTS 轮子。
- 容器以非 root 用户身份运行，UID 为 1000。在 Linux 上，如果挂载的目录不能被该 UID 写入，则无法写入缓存——添加 `--user "$(id -u):$(id -g)"` 或 `chown` 到该目录。Docker Desktop 在 macOS 和 Windows 上会自动处理此问题。

</details>

**渲染音频**需要 [`voice-soundboard`](https://pypi.org/project/voice-soundboard/) TTS 引擎（额外的 `[render]`）以及系统 PATH 路径下的 **FFmpeg**（`winget install ffmpeg` · `brew install ffmpeg` · `apt install ffmpeg`）。在渲染之前的所有步骤——解析、分配角色、编译、审查——都可以无需这些工具完成。运行 `audiobooker diagnose` 以检查您的设置。
...
### 输入和结构
- **EPUB、TXT、Markdown、PDF、DOCX**，或者一个**包含按章节划分的文件**的文件夹（Scrivener/Obsidian/连载小说）。
- **基于目录的 EPUB 分割**——从书籍本身的目录中提取章节边界和标题。
- **DOCX** 根据 Word `Heading 1/2`/`Title` 样式进行分割；**PDF** 检测标题（带有扫描 PDF 的保护机制）；自定义 `--chapter-delimiter`。
- 智能文本清理、支持 Markdown 的去除格式、脚注处理，以及一个**可重复使用的发音词典**（`pronunciation import/export`，CSV/JSON 格式，支持音素传递）。

### 角色分配和归属
- **多语音合成**，提供可解释的、排序的语音**建议**，以及一个**`audition`**命令，用于对每个角色进行 A/B 测试。
- **交互式角色分配**、按性别/角色进行**批量 `cast-fill`**、**可重用的命名角色预设**，可在整个系列中使用，以及**CSV 角色表**，供协作者使用。
...
WAV 没有章节原子，因此 WAV 渲染会明确说明这一点，而不是报告章节混合失败——当音频要输入到编辑器时，请使用它。
...
- **7 种语言配置文件**（en/fr/de/es/ja/it/pt）· **可插拔的 TTS 引擎**（`--engine`，入口点——支持 Piper/Coqui/ElevenLabs）· 大多数命令的可编写脚本的 `--json` · 结构化的退出代码。

## 母带处理以符合 ACX 音频规范

ACX 发布了一个精确、可测量的音频目标。这是有声读物领域最接近母带处理标准的标准，无论您对文件进行何种操作，都值得达到这个标准。

| 要求 | ACX 规范 | `--acx` 的作用 |
|---|---|---|
| 响度 | RMS 介于 **−23 和 −18 dBFS** 之间 | 两次传递 `loudnorm`，目标值为 −20 LUFS，这使得其值位于该范围内。 |
| 峰值 | 峰值应等于或低于 **−3 dBFS** | 在同一阶段进行强制执行 |
| 噪声底噪 | 噪声底噪应等于或低于 **−60 dBFS** | 进行测量和报告——绝不会在不告知的情况下“修复”。 |
...
**`master-check` 测量的是无权重的 RMS，而不是 LUFS。** 它们是不同的量，ACX 限制的是前者。−20 LUFS 的值是母带处理阶段*如何达到*该值的手段——这是 `ffmpeg loudnorm` 可以设置的目标——而不是之后进行检查的内容。

**噪声底噪是测量，而不是校正。** 这是最常导致失败的要求，并且它来自源音频。一个可以静默地抑制噪声的工具会隐藏您需要看到的一个数字。

### AI 叙述的有声读物实际上可以达到的范围

满足规范并不等同于被接受，这一点值得明确说明：**ACX 的标准提交流程适用于人类叙述。** 其 2026 年 4 月的要求清单中，未经授权的文本到语音和 AI 录音被列为不接受的内容，因此 AI 叙述的作品需要事先获得 ACX 的授权，而不是进行普通提交。

接受 AI 叙述的渠道，通常需要进行说明，包括亚马逊的 **Virtual Voice**，通过 KDP（仅限亚马逊分发），以及 **Spotify Audiobooks for Authors**、**Author's Republic** 和 **Kobo Writing Life** 等聚合平台。该领域的零售商政策变化迅速——请自行检查当前的条款，而不是依赖于本段。
...
| `cast-preset save\ | list\ | apply\ | delete` | 可重用的角色预设，用于在不同书籍中 |
| `cast-export` · `cast-import <file>` | 将演员阵容转换为 JSON/CSV 格式，以便进行双向传输——手动编辑，或在不同版本之间重复使用。 |
| `audition <char>` | 为单个角色（`--render`）提供 A/B 排序的候选配音。 |
| `compile` | 检测对话、识别说话者、推断情感。 |
...
默认引擎为 `voice-soundboard`，但可以通过 setuptools 入口点（`audiobooker.tts_engines`）切换合成后端。
...
`render(...)` 和 `compile(...)` 接受一个注入的 `engine=`（任何实现 `TTSEngine` 协议的对象）和一个进度回调——将有声读物制作器嵌入到 GUI 或服务中。
