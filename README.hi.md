...
**डॉकर** — इसमें पहले से ही एफएफएमपीईजी शामिल है, और हर रिलीज़ पर इसे जीएचसीआर पर प्रकाशित किया जाता है:
```bash
docker run --rm -v "$(pwd):/data" ghcr.io/mcp-tool-shop-org/audiobooker \
  make /data/mybook.epub --acx
```
यह एक माउंट पर्याप्त है, और `--rm` सुरक्षित है: रेंडर कैश प्रोजेक्ट फ़ाइल के बगल में स्थित होता है, न कि होम डायरेक्टरी में, इसलिए फिर से चलाने पर यह पूरी किताब को फिर से संश्लेषित करने के बजाय, पिछली प्रक्रिया से जारी रहता है।

<details>
<summary>Container details — tags, the cache, and file ownership</summary>

- `latest`, `2`, `2.1` और सटीक संस्करण टैग किए गए हैं, और हर रिलीज़ पर इन्हें जीएचसीआर पर पुश किया जाता है।
- एंट्रीपॉइंट `audiobooker` है, इसलिए इमेज नाम के तुरंत बाद सबकमांड पास करें — प्रोग्राम नाम को दोहराएं नहीं।
- कैश `<book-dir>/.audiobooker/cache` पर संग्रहीत होता है। यही कारण है कि एक बाइंड माउंट स्थायी भंडारण के लिए पर्याप्त है; इसे खोने का मतलब है कि पूरी टीटीएस प्रक्रिया को फिर से चलाना होगा, न कि केवल री-मक्सिंग करना।
- `/ext` एक वैकल्पिक दूसरा माउंट है, जिसका उपयोग केवल आपके स्वयं के टीटीएस व्हील को प्रदान करने के लिए किया जाता है।
- कंटेनर एक गैर-रूट यूआईडी 1000 के रूप में चलता है। लिनक्स पर, यदि माउंट की गई डायरेक्टरी उस यूआईडी द्वारा लिखने योग्य नहीं है, तो कैश नहीं लिखा जा सकता है — डायरेक्टरी में `--user "$(id -u):$(id -g)"` या `chown` जोड़ें। मैकओएस और विंडोज पर डॉकर डेस्कटॉप आपके लिए यह काम करता है।

</details>

**ऑडियो रेंडरिंग** के लिए [`voice-soundboard`](https://pypi.org/project/voice-soundboard/) टीटीएस इंजन (अतिरिक्त `[render]`) और पाथ पर **एफएफmpeg** (`winget install ffmpeg` · `brew install ffmpeg` · `apt install ffmpeg`) की आवश्यकता होती है। रेंडर करने से पहले - पार्स, कास्ट, कंपाइल, समीक्षा - यह सब बिना इनके काम करता है। अपने सेटअप की जांच करने के लिए `audiobooker diagnose` चलाएं।
...
- **डीओसीएक्स** वर्ड `Heading 1/2`/`Title` शैलियों पर विभाजित होता है; **पीडीएफ** शीर्षकों का पता लगाता है (स्कैन किए गए पीडीएफ के साथ), कस्टम `--chapter-delimiter`।
- स्मार्ट टेक्स्ट क्लीनिंग, मार्कडाउन-जागरूक स्ट्रिपिंग, पाद लेख हैंडलिंग, और एक **पुन: प्रयोज्य उच्चारण शब्दकोश** (`pronunciation import/export`, सीएसवी/जेएसओएन, जिसमें फोनम पासथ्रू हो)।
...
- **इंटरैक्टिव कास्टिंग**, लिंग/भूमिका के अनुसार **बल्क `cast-fill`**, श्रृंखला में पुन: प्रयोज्य **नाममात्र कास्ट प्रीसेट**, और सहयोगियों के लिए **सीएसवी कास्ट शीट**।
- **संवाद का पता लगाना + वक्ता विशेषता** (वैकल्पिक **बुकएनएलपी** सह-संदर्भ), **उपनाम ऑटो-डिस्कवरी**, और समायोज्य **तीव्रता**, **दृश्य-स्तर के मूड** और शैली **प्रीसेट पैक** के साथ **भावना अनुमान**।
...
डब्ल्यूएवी में कोई अध्याय एटम नहीं है, इसलिए डब्ल्यूएवी रेंडर स्पष्ट रूप से ऐसा बताता है, बजाय इसके कि विफल अध्याय मल्टीप्लेक्स की रिपोर्ट की जाए - जब ऑडियो किसी संपादक में जा रहा हो, तो इसका उपयोग करें।
- **एसीएक्स-विशिष्ट मास्टरी** (`--acx`) + एक **`master-check`** जो आरएमएस लाउडनेस, पीक और शोर स्तर पर पास/फेल की रिपोर्ट करता है; खुदरा **`sample`** क्लिप।
- समानांतर रेंडरिंग, एक **स्थायी रेंडर कैश** जिसमें फिर से शुरू करने की सुविधा, गतिशील प्रगति + ईटीए, और संरचित विफलता रिपोर्ट शामिल हैं।
...
**शोर स्तर को मापा जाता है, ठीक नहीं किया जाता है।** यह वह आवश्यकता है जो सबसे अधिक विफल होती है, और यह स्रोत ऑडियो से आती है। एक उपकरण जो इसे चुपचाप गेट करता है, वह उस एक नंबर को छिपा देगा जिसे आपको देखने की आवश्यकता है।

### जहां एक एआई-वर्णित ऑडियोबुक वास्तव में जा सकती है

विशिष्टताओं को पूरा करना स्वीकार किए जाने के समान नहीं है, और यह स्पष्ट करने योग्य है: **एसीएक्स का मानक सबमिशन प्रवाह मानव कथन के लिए है।** इसकी अप्रैल 2026 आवश्यकताओं की सूची में अनधिकृत टेक्स्ट-टू-स्पीच और एआई रिकॉर्डिंग शामिल हैं, जो उन चीजों में से हैं जिन्हें यह स्वीकार नहीं करता है, इसलिए एक एआई-वर्णित शीर्षक को एसीएक्स से पूर्व प्राधिकरण की आवश्यकता होती है, न कि एक सामान्य सबमिशन की।

ऐसे मार्ग जो आमतौर पर प्रकटीकरण के साथ एआई कथन को स्वीकार करते हैं, उनमें अमेज़ॅन के **वर्चुअल वॉयस** के माध्यम से केडीपी (केवल अमेज़ॅन वितरण) और **स्पोटिफाई ऑडियोबुक्स फॉर ऑथर्स**, **ऑथर्स रिपब्लिक** और **कोबो राइटिंग लाइफ** जैसे एग्रीगेटर शामिल हैं। इस क्षेत्र में खुदरा नीति तेजी से बदलती है - इस पैराग्राफ पर भरोसा करने के बजाय, वर्तमान नियमों की स्वयं जांच करें।
...
| कमांड | विवरण |
|---------|-------------|
| `make <file>` | वन-शॉट: नया → संकलित करें → ऑटो-कास्ट → रेंडर करें |
...
| `render` | ऑडियोबुक प्रस्तुत करें (`--acx`, `--format`, `--split`, `--bitrate`, `--engine`, `--watch`, `--cover`, `-j N`) |
| `sample` · `master-check <file>` | मास्टर खुदरा नमूना · ACX ऑडियो विनिर्देश के विरुद्ध जांच करें |
| `export-chapters` · `podcast` | अध्याय क्यू शीट (ffmetadata/cue/json) · पॉडकास्ट आरएसएस फ़ीड |
| `preview` · `batch` · `diagnose` | वॉइस क्यूए क्लिप · बैच/`--manifest` · पर्यावरण जांच (यदि बॉक्स प्रस्तुत नहीं कर सकता है तो गैर-शून्य मान देता है) |
| `load <file>` | एक मौजूदा `.audiobooker` प्रोजेक्ट खोलें |
| `voices` · `chapters` · `speakers` · `info` · `status` · `cache` · `emotions` · `pronunciation` · `completion` | जांचें और प्रबंधित करें |

प्रत्येक कमांड `-h/--help` का समर्थन करता है। वैश्विक ध्वज: `--silent`, `--debug`। **निकास कोड:** `0` ठीक · `1` उपयोगकर्ता त्रुटि (जिसमें एक ऐसी पुस्तक भी शामिल है जो संकलित नहीं होगी, या एक प्रस्तुतीकरण अस्वीकार कर दिया गया क्योंकि विशेषता विफल रही) · `2` रनटाइम · `3` आंशिक (बैच)।

## कॉन्फ़िगरेशन

बार-बार ध्वज पास करने के बजाय एक बार डिफ़ॉल्ट सेट करें — `.audiobookerrc` (TOML) अपनी पुस्तक के बगल में, या `pyproject.toml` में `[tool.audiobooker]`। प्राथमिकता है **CLI ध्वज > प्रोजेक्ट कॉन्फ़िगरेशन > उपयोगकर्ता कॉन्फ़िगरेशन (`~/.audiobookerrc`) > अंतर्निहित डिफ़ॉल्ट**।
...
`render(...)` और `compile(...)` एक इंजेक्टेड `engine=` (किसी भी ऑब्जेक्ट को लागू करने वाला `TTSEngine` प्रोटोकॉल) और एक प्रगति कॉलबैक स्वीकार करते हैं — ऑडियोबुकर को एक जीयूआई या सेवा में एम्बेड करें।
