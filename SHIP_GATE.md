...
- [x] `[cli]` `--help` output accurate for all commands and flags — **MEASURED 2026-09-14 by `tools/check_help_accuracy.py`, not asserted.** 34 subcommands, 62 real flags, 47 documented across README and the handbook, **0 documented-but-nonexistent**. The one that was broken: the handbook gave `cast` a `--speed` flag that exists nowhere — `speed` is a CONFIG FILE key, and an alias for `global_speed`, so it is global rather than per-character as the table implied. Fixed alongside three other handbook claims the code disproves (two pages still said English was the only language profile; seven ship). The `render --watch` string named in the previous evidence was corrected earlier in this session.
      The check is a SCRIPT rather than a date, which is the actual lesson here: the prior evidence read "verified in CI, 2026-02-27" and was still sitting there after two-thirds of the CLI had been added. Re-run `python tools/check_help_accuracy.py`; it exits non-zero when a documented flag does not exist. It also lists real-but-undocumented flags (17 today) — a gap worth closing, but not a lie, so it does not fail the gate.
...
- [x] `[all]` Version in manifest matches git tag (2026-09-14) — pyproject 3.0.0 and npm/package.json 3.0.0 agree, and `tests/test_version.py` asserts the installed distribution metadata matches pyproject rather than trusting it (that test caught a stale editable install during this very bump, which is the whole reason it exists). The v3.0.0 tag is cut from the release commit that carries this line, so the claim is true at the tag and not before it. The prior note said "v2.0.0 tag pending" and it never landed: `git tag -l` shows v0.2.0, v0.5.1, v0.5.2, v2.0.1, v2.1.0, v2.1.1 — **no v1.0.0 and no v2.0.0**, though CHANGELOG documents both as shipped releases (0.3.0/0.4.0/0.5.0 are likewise untagged). That historical CHANGELOG/tag divergence is real and tracked separately rather than papered over here.
